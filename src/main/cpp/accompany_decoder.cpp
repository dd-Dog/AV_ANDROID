...
int AccompanyDecoder::readSamples(short *samples, int size) {
    LOGI("readSamples,size=" + size);
    if (seek_req) {
        audioBufferCursor = audioBufferSize;
    }
    int sampleSize = size;
    while (size > 0) {
        if (audioBufferCursor < audioBufferSize) { //下标位置落后缓存大小，说明有数据待拷贝
            ...
            size -= copySize;   //最终size会被减为0，退出循环
...
//				LOGI(" avcodec_decode_audio4 ...", duration);
                ...
                if (gotframe) {
                    ...
                    if (swrContext) {
                        ...
                         * @param nb_channels 频道数
                         * @param nb_samples 单个通道中的样本数
                         * @param sample_fmt 样本格式
                         * @param 对齐缓冲区大小对齐（0 =默认，1 =无对齐）
                         * @return 需要缓冲区大小，或失败时出现负错误代码
                        */
                        ...
                        if (!swrBuffer || swrBufferSize < bufSize) {
                            ...
                            //更改已经配置的内存空间，即更改由malloc()函数分配的内存空间的大小，重新返回指针
                    ...
                    audioBufferSize = numFrames * numChannels;  //本次读取的数据大小
					LOGI("duration is %.6f position is %.6f audioBufferSize is %d\n", duration, position, audioBufferSize);
